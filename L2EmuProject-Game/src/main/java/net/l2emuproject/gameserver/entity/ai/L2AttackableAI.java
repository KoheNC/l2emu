...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target instanceof L2Playable)
		{
			...
			if (!(me instanceof L2Boss) && !(me.canSeeThroughSilentMove()) && ((L2Playable) target).isSilentMoving())
				return false;
		}
		...
		if (target instanceof L2Player)
		{
			...
			if (me.getFactionId().equals("beleth") && player.getPlayerTransformation().isTransformed() && player.getPlayerTransformation().getTransformationId() == 101)
				return false;
			...
			for (L2Object obj : _actor.getKnownList().getKnownObjects().values())
			{
				if (obj instanceof L2Npc)
				{
					...
					if ((faction_id.equals("c_dungeon_clan")) &&
						((npcfaction.equals("c_dungeon_lilim")) || npcfaction == "c_dungeon_nephi"))
					...
					if (_actor.isInsideRadius(npc, npc.getFactionRange() + npc.getTemplate().getCollisionRadius(), true, false) && npc.getAI() != null)
					{
						...
						{
							if ((originalAttackTarget instanceof L2Player) || (originalAttackTarget instanceof L2Summon))
							{
								L2Player player = (originalAttackTarget instanceof L2Player) ? (L2Player) originalAttackTarget : ((L2Summon) originalAttackTarget)
										.getOwner();
