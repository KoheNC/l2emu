...
import static net.l2emuproject.gameserver.entity.ai.CtrlIntention.AI_INTENTION_PICK_UP;
...
public class L2CharacterAI extends AbstractAI
{
	...
	/* (non-Javadoc)
	 * @see net.l2emuproject.gameserver.ai.AbstractAI#onIntentionMoveToInABoat(net.l2emuproject.gameserver.model.L2CharPosition, net.l2emuproject.gameserver.model.L2CharPosition)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see net.l2emuproject.gameserver.ai.AbstractAI#onIntentionMoveToInAirShip(net.l2emuproject.gameserver.model.L2CharPosition, net.l2emuproject.gameserver.model.L2CharPosition)
	 */
	@Override
	...
	protected boolean maybeMoveToPawn(L2Object target, int offset)
	{
		...
		if (distance > offset)
		{
			...
			if (getIntention() == CtrlIntention.AI_INTENTION_CAST &&
					_actor instanceof L2Player && ((L2Player)_actor).getPlayerTransformation().isTransformed())
			{
				if (!((L2Player)_actor).getPlayerTransformation().getTransformation().canStartFollowToCast())
				{
					_actor.sendPacket(SystemMessageId.DIST_TOO_FAR_CASTING_STOPPED);
					_actor.sendPacket(ActionFailed.STATIC_PACKET);
			...
			// If not running, set the L2Character movement type to run and send Server->Client packet ChangeMoveType to all others L2Player
			if (!_actor.isRunning() && !(this instanceof L2PlayerAI) && !(this instanceof L2SummonAI))
				_actor.setRunning();
