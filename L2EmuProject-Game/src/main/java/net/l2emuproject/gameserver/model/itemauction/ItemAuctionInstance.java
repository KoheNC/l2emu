...
{
	static final Log								_log				= LogFactory.getLog(ItemAuctionInstance.class);
	...
	public ItemAuctionInstance(final int instanceId, final AtomicInteger auctionIds, final Node node) throws Exception
	{
		...
		for (Node na = node.getFirstChild(); na != null; na = na.getNextSibling())
		{
			...
			{
				_log.warn("ItemAuctionInstance: Failed loading auction item", e);
			}
		}
		...
		try
		{
			...
			while (rset.next())
			{
				...
				{
					_log.warn("ItemAuctionInstance: Failed loading auction: " + auctionId, e);
				}
			}
		}
		catch (final SQLException e)
		{
			_log.error("L2ItemAuctionInstance: Failed loading auctions.", e);
			return;
		}
		finally
		{
		...
		_log.info("L2ItemAuctionInstance: Loaded " + _items.size() + " item(s) and registered " + _auctions.size() + " auction(s) for instance " + _instanceId
				+ ".");
	...
	final void checkAndSetCurrentAndNextAuction()
	{
		...
		if (currentAuction != null && currentAuction.getAuctionState() != ItemAuctionState.FINISHED)
		{
			...
			_log.info("L2ItemAuctionInstance: Schedule current auction " + currentAuction.getAuctionId() + " for instance " + _instanceId);
		}
		else
		{
			...
			_log.info("L2ItemAuctionInstance: Schedule next auction " + nextAuction.getAuctionId() + " on "
	...
	private final class ScheduleAuctionTask implements Runnable
	{
		...
		public final void run()
		{
			...
			{
				_log.error("L2ItemAuctionInstance: Failed scheduling auction " + _auction.getAuctionId(), e);
			}
		}
			...
			switch (state)
			{
				case CREATED:
				{
					if (!_auction.setAuctionState(state, ItemAuctionState.STARTED))
						throw new IllegalStateException("Could not set auction state: " + ItemAuctionState.STARTED.toString() + ", expected: "
								+ state.toString());

					_log.info("L2ItemAuctionInstance: Auction " + _auction.getAuctionId() + " has started for instance " + _auction.getInstanceId());
					checkAndSetCurrentAndNextAuction();
					break;
				}
				...
				case STARTED:
				{
					...
					if (!_auction.setAuctionState(state, ItemAuctionState.FINISHED))
						throw new IllegalStateException("Could not set auction state: " + ItemAuctionState.FINISHED.toString() + ", expected: "
								+ state.toString());
	...
	final void onAuctionFinished(final ItemAuction auction)
	{
		...
		if (bid != null)
		{
			...
			if (player != null)
			{
				...
				_log.info("L2ItemAuctionInstance: Auction " + auction.getAuctionId() + " has finished. Highest bid by " + player.getName() + " for instance "
						+ _instanceId);
			}
			else
			{
				...
				_log.info("L2ItemAuctionInstance: Auction " + auction.getAuctionId() + " has finished.");
			}
		...
		{
			_log.info("L2ItemAuctionInstance: Auction " + auction.getAuctionId() + " has finished. There have not been any bid for instance " + _instanceId);
		}
	}
	...
	private final ItemAuction loadAuction(final int auctionId) throws SQLException
	{
		...
		try
		{
			...
			PreparedStatement statement = con
					.prepareStatement("SELECT auctionItemId,startingTime,endingTime,auctionStateId FROM item_auction WHERE auctionId=?");
			statement.setInt(1, auctionId);
			ResultSet rset = statement.executeQuery();

			if (!rset.next())
			{
				_log.warn("ItemAuctionInstance: Auction data not found for auction: " + auctionId);
				return null;
			}
			...
			{
				_log.warn("ItemAuctionInstance: Invalid starting/ending paramaters for auction: " + auctionId);
				return null;
			}
			...
			if (auctionItem == null)
			{
				_log.warn("ItemAuctionInstance: AuctionItem: " + auctionItemId + ", not found for auction: " + auctionId);
				return null;
			}
			...
			{
				_log.warn("ItemAuctionInstance: Invalid auctionStateId: " + auctionStateId + ", for auction: " + auctionId);
				return null;
			}
			...
			{
				_log.info("ItemAuctionInstance: Clearing expired auction: " + auctionId);
