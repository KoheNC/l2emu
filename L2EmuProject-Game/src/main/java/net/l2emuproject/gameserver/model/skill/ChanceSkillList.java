...
public final class ChanceSkillList
{
	...
	public void onHit(L2Character evtInitiator, int damage, boolean ownerWasHit, boolean wasCrit, boolean wasRange)
	{
		int event;
		if (ownerWasHit)
		{
	...
	public void onSkillHit(L2Character evtInitiator, boolean ownerWasHit, L2Skill trigger)
	{
		...
		onEvent(event, 0, evtInitiator, trigger.getElement());
	}
	...
	public void onEvent(int event, int damage, L2Character evtInitiator, byte element)
	{
		final boolean playable = evtInitiator instanceof L2Playable;
		for (IChanceSkillTrigger trigger : _triggers)
		{
			if (trigger != null && trigger.getChanceCondition().trigger(event, damage, element, playable))
			{
