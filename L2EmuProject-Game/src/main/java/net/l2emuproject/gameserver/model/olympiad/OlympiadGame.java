 ...
 */
package net.l2emuproject.gameserver.model.olympiad;
...
import net.l2emuproject.gameserver.network.serverpackets.ExOlympiadMatchEnd;
...
public class OlympiadGame
{
	...
	protected OlympiadGame(int id, COMP_TYPE type, FastList<L2PcInstance> list)
	{
		...
		if (list != null)
		{
			...
			try
			{
				...
				_playerOne.getPlayerOlympiad().setOlympiadGameId(id);
				_playerTwo.getPlayerOlympiad().setOlympiadGameId(id);
	...
	protected boolean portPlayersToArena()
	{
		...
		try
		{
			...
			_playerOne.getPlayerOlympiad().setIsInOlympiadMode(true);
			_playerOne.getPlayerOlympiad().setIsOlympiadStart(false);
			_playerOne.getPlayerOlympiad().setOlympiadSide(1);
			_playerOne.getPlayerOlympiad().olyBuff = 5;

			_playerTwo.getPlayerOlympiad().setIsInOlympiadMode(true);
			_playerTwo.getPlayerOlympiad().setIsOlympiadStart(false);
			_playerTwo.getPlayerOlympiad().setOlympiadSide(2);
			_playerTwo.getPlayerOlympiad().olyBuff = 5;
	...
	protected boolean haveWinner()
	{
		...
		try
		{
			if (_playerOne != null && _playerOne.getPlayerOlympiad().getOlympiadGameId() != -1)
			{
		...
		try
		{
			if (_playerTwo != null && _playerTwo.getPlayerOlympiad().getOlympiadGameId() != -1)
			{
	...
	protected boolean makeCompetitionStart()
	{
		...
		try
		{
			for (L2PcInstance player : _players)
			{
				player.getPlayerOlympiad().setIsOlympiadStart(true);
			}
		}
		catch (Exception e)
		{
...
class OlympiadGameTask implements Runnable
{
	...
	protected boolean checkDefaulted()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (player != null)
				player.getPlayerOlympiad().setOlympiadGameId(_game._stadiumID);
