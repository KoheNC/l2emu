...
public class Quest extends ManagedScript
{
	...
	// TODO: Rework
	public Quest(int questId, String name, String descr, String folder)
	{
		_questId = questId;
		_name = name;
		_descr = descr;
	...
	{
		ON_FIRST_TALK(true), // control the first dialog shown by NPCs when they are clicked (some quests must override the default npc action)
	...
	public final boolean notifySkillSee(L2Npc npc, L2Player caster, L2Skill skill, L2Object[] targets, boolean isPet)
	{
		String res = null;
		try
		{
	...
	{
		String content = HtmCache.getInstance().getHtm("data/scripts/" + getFolder().toLowerCase() + "/" + getName() + "/" + fileName);
	...
	*/
	...
	{
		return getRandomPartyMember(player, CONDITION, value);
	}
	...
	public String showHtmlFile(L2Player player, String fileName)
	{
		...
		String directory = getFolder().toLowerCase();
