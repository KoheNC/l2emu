 ...
 */
package net.l2emuproject.gameserver.services.party;
...
import net.l2emuproject.gameserver.events.global.dimensionalrift.DimensionalRift;
...
import net.l2emuproject.gameserver.network.serverpackets.ExAskModifyPartyLooting;
...
import net.l2emuproject.gameserver.network.serverpackets.ExSetPartyLooting;
import net.l2emuproject.gameserver.network.serverpackets.L2GameServerPacket;
import net.l2emuproject.gameserver.network.serverpackets.PartySmallWindowAdd;
import net.l2emuproject.gameserver.network.serverpackets.PartySmallWindowAll;
...
public final class L2Party
{
	 ...
	 * @return
	 */
	private L2Player getCheckedRandomMember(int ItemId, L2Character target)
	{
	 ...
	 * @return
	 */
	private L2Player getCheckedNextLooter(int ItemId, L2Character target)
	{
	 ...
	 * @return
	 */
	private L2Player getActualLooter(L2Player player, int ItemId, boolean spoil, L2Character target)
	{
    ...
    */
	 ...
	 * @param adena
	 */
	public void distributeAdena(L2Player player, long adena, L2Character target)
	{
				...
				if (validMembers.contains(member))
				{
					...
					if (!member.isDead())
					{
						...
						if (member instanceof L2Player)
						{
							...
							if (addexp > 0)
								((L2Player)member).getPlayerVitality().updateVitalityPoints(vitalityPoints, true, false);
						}
						else
	...
	public final void requestLootChange(byte type)
	{
		if (_requestChangeLoot != -1)
		{
		...
		int additionalTime = L2Player.REQUEST_TIMEOUT * 3000;
	...
	public synchronized final void answerLootChangeRequest(L2Player member, boolean answer)
	{
		if (_requestChangeLoot == -1)
			return;
	...
	private synchronized final void finishLootRequest(boolean success)
	{
		if (_requestChangeLoot == -1)
			return;
		if (_checkTask != null)
	...
	private final class ChangeLootCheck implements Runnable
	{
		@Override
