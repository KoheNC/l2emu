...
import net.l2emuproject.gameserver.manager.leaderboards.FishermanManager;
...
public class L2Fishing implements Runnable
{
	...
	public synchronized void doDie(boolean win)
	{
		...
		_fisher.getPlayerFish().endFishing(win);
		_fisher = null;
	}
	...
	private void spawnPenaltyMonster()
	{
		...
		if (temp != null)
		{
			try
			{
				...
				spawn.setLocx(_fisher.getPlayerFish().getFishx());
				spawn.setLocy(_fisher.getPlayerFish().getFishy());
				spawn.setLocz(_fisher.getPlayerFish().getFishz());
				spawn.setAmount(1);
