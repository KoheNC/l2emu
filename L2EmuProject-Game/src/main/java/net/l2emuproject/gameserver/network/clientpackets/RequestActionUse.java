...
public class RequestActionUse extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		case 22: // pet attack
			if (target != null && pet != null && pet != target && activeChar != target && !pet.isAttackingDisabled() && !pet.isOutOfControl())
			{
				...
				if (target.getActingPlayer() != null && pet.getOwner().getSiegeState() > 0 && pet.getOwner().isInsideZone(L2Zone.FLAG_SIEGE)
		...
		case 1088: // Fatal Claw (Panther)
			useSkill(6096);
			break;
		case 1089: // Deinonychus - Tail Strike CT 2.4
			useSkill(6199);
			break;
		case 1090: // Guardian's Strider - Strider Bite CT 2.4
			useSkill(6205);
			break;
		case 1091: // Guardian's Strider - Strider Fear CT 2.4
			useSkill(6206);
			break;
		case 1092: // Guardian's Strider - Strider Dash CT 2.4
			useSkill(6207);
			break;
	...
	private final void useSocial(int socialId, L2Player activeChar)
	{
		...
		{
			activeChar.broadcastPacket(new SocialAction(activeChar, socialId));
		}
	}
