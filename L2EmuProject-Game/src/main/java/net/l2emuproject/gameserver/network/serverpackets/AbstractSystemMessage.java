...
public abstract class AbstractSystemMessage<T extends AbstractSystemMessage> extends L2GameServerPacket
{
	...
	private static final class SystemStringElement extends Element
	{
		private final int _messageId;
		
		private SystemStringElement(int messageId)
		{
		...
		protected int getType()
		{
			return TYPE_SYSTEM_STRING;
		}

		@Override
		protected void write2(AbstractSystemMessage sm)
		{
			sm.writeD(_messageId);
		}
	}
...
//	private static final int TYPE_ELEMENT_NAME	= 9;
	...
	private void addElement(Element element)
	{
		...
		if (_log.isDebugEnabled())
			_log.debug("AbstractSystemMessage: Too much parameter for ID: " + _messageId, new ArrayIndexOutOfBoundsException());
	...
	public void prepareToSend(L2GameClient client, L2PcInstance activeChar)
	{
		...
		if (_log.isDebugEnabled())
			_log.debug("AbstractSystemMessage: Empty parameter for ID: " + _messageId, new ArrayIndexOutOfBoundsException());
	}
	...
	public T addSystemString(int id)
	{
		if (checkNPE(id))
			return (T)this;
		
		addElement(new SystemStringElement(id));
