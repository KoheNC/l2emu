...
public final class UserInfo extends L2GameServerPacket
{
	...
	protected void writeImpl()
	{
		...
		writeC(_activeChar.getPlayerFish().isFishing() ? 1 : 0); //Fishing Mode
		writeD(_activeChar.getPlayerFish().getFishx()); //fishing x
		writeD(_activeChar.getPlayerFish().getFishy()); //fishing y
		writeD(_activeChar.getPlayerFish().getFishz()); //fishing z
