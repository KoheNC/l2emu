...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (!getClient().getFloodProtector().tryPerformAction(Protected.USEITEM))
			return;
		...
		if (activeChar.getPlayerFish().isFishing() && !ShotTable.isFishingShot(itemId))
		{
		...
		if (item.isEquipable())
		{
			...
			if (activeChar.getPlayerOlympiad().isInOlympiadMode() && (item.isHeroItem() || item.isOlyRestrictedItem()))
			{
