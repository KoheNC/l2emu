	...
	private Vector<L2Item> sortItemsByType(Vector<L2Item> items)
	{
		...
		for (L2WeaponType type : L2WeaponType.values())
		{
			...
			for (L2Item item : items)
			{
				...
				else if (!(item instanceof net.l2emuproject.gameserver.templates.item.L2Weapon))
					continue;

				L2WeaponType weaponType = ((net.l2emuproject.gameserver.templates.item.L2Weapon) item).getItemType();
