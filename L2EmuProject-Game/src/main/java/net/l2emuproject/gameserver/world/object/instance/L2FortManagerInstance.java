...
public class L2FortManagerInstance extends L2MerchantInstance
{
		...
		else if (condition == COND_OWNER)
		{
			...
			if (actualCommand.equalsIgnoreCase("banish_foreigner"))
			{
				...
				if (L2Clan.checkPrivileges(player, L2Clan.CP_CS_DISMISS))
				{
					if (val.isEmpty())
					{
						html.setFile("data/npc_data/html/fortress/foreman-expel.htm");
					}
					else
					{
						getFort().banishForeigners(); // Move non-clan members off fortress area
						html.setFile("data/npc_data/html/fortress/foreman-expeled.htm");
					}
				}
				else
				{
					html.setFile("data/npc_data/html/fortress/foreman-noprivs.htm");
				}
			...
			else if(actualCommand.equalsIgnoreCase("manage_vault"))
			{
				if (L2Clan.checkPrivileges(player, L2Clan.CP_CL_VIEW_WAREHOUSE))
				{
					...
					{
						NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
						html.setFile("data/npc_data/html/fortress/foreman-vault.htm");
			...
			else if (actualCommand.equalsIgnoreCase("receive_report"))
			{
				...
				if (getFort().getFortState() == 1)
					html.setFile("data/npc_data/html/fortress/foreman-report-independant.htm");
				else if (getFort().getFortState() == 2)
					html.setFile("data/npc_data/html/fortress/foreman-report-contracted.htm");
				else
					html.setFile("data/npc_data/html/fortress/foreman-report-unknown.htm");
			...
			else if (actualCommand.equalsIgnoreCase("operate_door")) // Door Control
			{
				if (L2Clan.checkPrivileges(player, L2Clan.CP_CS_OPEN_DOOR))
				{
					...
					NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
					html.setFile("data/npc_data/html/fortress/" + getTemplate().getNpcId() + "-d.htm");
					html.replace("%objectId%", String.valueOf(getObjectId()));
			...
			{
				NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile("data/npc_data/html/fortress/foreman-manage.htm");
				html.replace("%objectId%", String.valueOf(getObjectId()));
				player.sendPacket(html);
				return;
			}
			else if (actualCommand.equalsIgnoreCase("functions"))
			{
				if (val.equalsIgnoreCase("tele"))
				{
					...
					if (getFort().getFunction(Fort.FUNC_TELEPORT) == null)
						html.setFile("data/npc_data/html/fortress/foreman-nac.htm");
					else
						html.setFile("data/npc_data/html/fortress/"+ getNpcId()+ "-t"
				...
				else if (val.equalsIgnoreCase("support"))
				{
					...
					{
						html.setFile("data/npc_data/html/fortress/support"
				...
				{
					NpcHtmlMessage html = new NpcHtmlMessage(1);
					html.setFile("data/npc_data/html/fortress/foreman-functions.htm");
				...
				if (L2Clan.checkPrivileges(player, L2Clan.CP_CS_SET_FUNCTIONS))
				{
					if (val.equalsIgnoreCase("recovery"))
					{
						if (st.countTokens() >= 1)
						{
							...
							{
								NpcHtmlMessage html = new NpcHtmlMessage(1);
								html.setFile("data/npc_data/html/fortress/functions-cancel.htm");
							...
							else if (val.equalsIgnoreCase("edit_hp"))
							{
								val = st.nextToken();
								NpcHtmlMessage html = new NpcHtmlMessage(1);
								html.setFile("data/npc_data/html/fortress/functions-apply.htm");
							...
							else if (val.equalsIgnoreCase("hp"))
							{
								if (st.countTokens() >= 1)
								{
									...
									val = st.nextToken();
									NpcHtmlMessage html = new NpcHtmlMessage(1);
									html.setFile("data/npc_data/html/fortress/functions-apply_confirmed.htm");
									if (getFort().getFunction(Fort.FUNC_RESTORE_HP) != null)
									{
										if (getFort().getFunction(Fort.FUNC_RESTORE_HP).getLvl() == Integer.valueOf(val))
										{
											html.setFile("data/npc_data/html/fortress/functions-used.htm");
									...
									switch (percent)
									{
										case 0:
											fee = 0;
											html.setFile("data/npc_data/html/fortress/functions-cancel_confirmed.htm");
											break;
										case 300:
									...
									{
										html.setFile("data/npc_data/html/fortress/low_adena.htm");
										sendHtmlMessage(player, html);
									}
									sendHtmlMessage(player, html);
								}
						...
						NpcHtmlMessage html = new NpcHtmlMessage(1);
						html.setFile("data/npc_data/html/fortress/edit_recovery.htm");
					...
					else if (val.equalsIgnoreCase("other"))
					{
						...
						NpcHtmlMessage html = new NpcHtmlMessage(1);
						html.setFile("data/npc_data/html/fortress/edit_other.htm");
					...
					{
						NpcHtmlMessage html = new NpcHtmlMessage(1);
						html.setFile("data/npc_data/html/fortress/manage.htm");
						sendHtmlMessage(player, html);
					}
				}
				else
				...
				try
				{
					...
					try
					{
						...
						else
						{
							...
							{
								html.setFile("data/npc_data/html/fortress/support-no_mana.htm");
								html.replace("%mp%", String.valueOf((int)getStatus().getCurrentMp()));
								sendHtmlMessage(player, html);
								return;
							}
						}
						html.setFile("data/npc_data/html/fortress/support-done.htm");
						html.replace("%mp%", String.valueOf((int)getStatus().getCurrentMp()));
						sendHtmlMessage(player, html);
					}
					catch (Exception e)
	...
	public final void showChatWindow(L2Player player)
	{
		...
		String filename = "data/npc_data/html/fortress/foreman-no.htm";

		final int condition = validateCondition(player);
		if (condition > COND_ALL_FALSE)
		{
			if (condition == COND_BUSY_BECAUSE_OF_SIEGE)
				filename = "data/npc_data/html/fortress/foreman-busy.htm"; // Busy because of siege
			else if (condition == COND_OWNER) // Clan owns Fortress
				filename = "data/npc_data/html/fortress/foreman.htm"; // Owner message window
		}
	...
	private void doTeleport(L2Player player, int val)
	{
		...
		if (list != null)
		{
			...
			{
				if (_log.isDebugEnabled())
					_log.warn("Teleporting player " + player.getName()
