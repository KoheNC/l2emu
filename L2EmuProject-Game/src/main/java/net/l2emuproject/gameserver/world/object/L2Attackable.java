...
import net.l2emuproject.gameserver.model.actor.status.AttackableStatus;
import net.l2emuproject.gameserver.model.actor.status.CharStatus;
import net.l2emuproject.gameserver.model.base.SoulCrystal;
...
import net.l2emuproject.gameserver.world.knownlist.AttackableKnownList;
...
public class L2Attackable extends L2Npc
{
	 ...
	 */
	private final Map<L2Player, AbsorberInfo> _absorbersList = new SingletonMap<L2Player, AbsorberInfo>().shared();
				...
				for (FastMap.Entry<L2Character, RewardInfo> entry = rewards.head(), end = rewards.tail(); (entry = entry.getNext()) != end;)
				{
					...
					if (attackerParty == null)
					{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							if (Config.ALLOW_DYNAMIC_RATES)
							{
								if (attacker.getLevel() <= 20)
								{
									exp *= Config.NO_GRADE_XP_BONUS;
									sp *= Config.NO_GRADE_SP_BONUS;
								}
								else if (attacker.getLevel() <= 40 && attacker.getLevel() >= 20)
								{
									exp *= Config.D_GRADE_XP_BONUS;
									sp *= Config.D_GRADE_SP_BONUS;
								}
								else if (attacker.getLevel() <= 52 && attacker.getLevel() >= 40)
								{
									exp *= Config.C_GRADE_XP_BONUS;
									sp *= Config.C_GRADE_SP_BONUS;
								}
								else if (attacker.getLevel() <= 61 && attacker.getLevel() >= 52)
								{
									exp *= Config.B_GRADE_XP_BONUS;
									sp *= Config.B_GRADE_SP_BONUS;
								}
								else if (attacker.getLevel() <= 76 && attacker.getLevel() >= 61)
								{
									exp *= Config.A_GRADE_XP_BONUS;
									sp *= Config.A_GRADE_SP_BONUS;
								}
								else if (attacker.getLevel() <= 80 && attacker.getLevel() >= 76)
								{
									exp *= Config.S_GRADE_XP_BONUS;
									sp *= Config.S_GRADE_SP_BONUS;
								}
								else if (attacker.getLevel() <= 85 && attacker.getLevel() >= 80)
								{
									exp *= Config.S80_GRADE_XP_BONUS;
									sp *= Config.S80_GRADE_SP_BONUS;
								}
							}
							// L2EMU_ADD
							...
							if (!attacker.isDead())
							{
								...
								if (attacker instanceof L2Player && !(this instanceof L2ChestInstance))
								{
									...
									if (addexp > 0)
										((L2Player)attacker).getPlayerVitality().updateVitalityPoints(getVitalityPoints(damage), true, false);
								}
								else
	...
	{
		if (target == null || getAggroList().isEmpty())
			return 0;

		AggroInfo ai = getAggroList().get(target);
		if (ai == null)
			return 0;
		if (target instanceof L2Player && (((L2Player)target).getAppearance().isInvisible() || target.isInvul()))
		{
	 ...
	 *            chance)
	 */
	private RewardItem calculateRewardItem(L2Player lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
	...
	private long calculateDropChance(double chance, L2DropData drop, boolean isSweep, int levelModifier)
	{
		...
		// L2EMU_ADD
		else if (drop != null && ItemTable.isCommonItem(drop.getItemId()))
		{
			chance *= Config.RATE_DROP_COMMON_ITEMS;
		}
		// L2EMU_ADD
		...
		else
		{
			...
			// L2EMU_ADD
			else if (drop != null && Config.LIST_OF_SPECIAL_ITEMS.contains(drop.getItemId()))
				chance *= Config.RATE_DROP_SPECIAL_ITEMS;
			// L2EMU_ADD
	 ...
	 *            chance)
	 */
	private RewardItem calculateCategorizedRewardItem(L2Player lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
	 ...
	 * the L2Character that has killed the L2Attackable is not a L2Player,
	...
	private class CommandChannelTimer implements Runnable
	{
		...
		public void run()
		{
			...
			for (L2Character player : getAggroList().keySet())
			{
