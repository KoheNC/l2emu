...
public final class ModuleTester extends Config
{
	...
	{
		Config.load();
		Config.DATAPACK_ROOT = new File("../L2EmuProject-DataPack");
	...
	{
		for (final File f : new File("../L2EmuProject-DataPack/data/stats/skills/").listFiles())
		{
			if (f.isHidden())
				continue;
	...
	private static final class SkillXMLConverter extends ContentConverter
	{
		...
		private ArrayList<String> convertSkill(ArrayList<String> list)
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				final String name = Attributes.getElementName(Byte.parseByte(getAttributeValue(line, "val")));
