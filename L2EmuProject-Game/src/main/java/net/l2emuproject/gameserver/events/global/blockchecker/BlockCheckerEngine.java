...
import net.l2emuproject.gameserver.network.serverpackets.ExCubeGameCloseUI;
import net.l2emuproject.gameserver.network.serverpackets.ExCubeGameEnd;
...
import net.l2emuproject.gameserver.system.restriction.global.BlockCheckerRestriction;
...
{
	private static final Log									_log				= LogFactory.getLog(BlockCheckerEngine.class);
	...
	private static final int[][]								ARENA_COORDINATES	=
																					{
	 ...
	 * @param team
	 */
	public void increasePlayerPoints(L2Player player, int team)
	{
		if (player == null)
			return;
	...
	{
		for (L2Player p : _holder.getAllPlayers())
		{
			p.sendPacket(new RelationChanged(plr, plr.getRelation(p), p));
		}
	}
	...
	public class StartEvent implements Runnable
	{
		...
		private void setUpPlayers()
		{
			...
			for (L2Player player : _holder.getAllPlayers())
			{
				...
				int x = ARENA_COORDINATES[_arena][tc];
				int y = ARENA_COORDINATES[_arena][tc + 1];
				...
				player.setInsideZone(L2Zone.FLAG_PVP, true);
				// Send needed packets
			...
			BlockCheckerRestriction.getInstance().activate();
		}
		...
		public void run()
		{
			...
			try
			{
				...
				for (int i = 0; i < _numOfBoxes; i++)
				{
					...
					spawn.setLocx(ARENA_COORDINATES[_arena][4] + Rnd.get(-400, 400));
					spawn.setLocy(ARENA_COORDINATES[_arena][5] + Rnd.get(-400, 400));
					spawn.setLocz(Z_COORD);
					spawn.setAmount(1);
					spawn.setHeading(1);
			...
			if (_round == 1 || _round == 2)
			{
				...
				try
				{
					...
					girlSpawn.setLocx(ARENA_COORDINATES[_arena][4] + Rnd.get(-400, 400));
					girlSpawn.setLocy(ARENA_COORDINATES[_arena][5] + Rnd.get(-400, 400));
					girlSpawn.setLocz(Z_COORD);
	...
	private class CountDown implements Runnable
	{
		...
		public void run()
		{
			_holder.broadCastPacketToTeam(new SystemMessage(SystemMessageId.BLOCK_CHECKER_ENDS_5));
	...
	private class EndEvent implements Runnable
	{
		...
		private void rewardPlayers()
		{
			...
			if (_isRedWinner)
			{
				...
				SystemMessage msg = new SystemMessage(SystemMessageId.TEAM_C1_WON);
		...
		private void rewardAsWinner(boolean isRed)
		{
			...
			for (Entry<L2Player, Integer> entry : tempPoints.entrySet())
			{
				L2Player pc = entry.getKey();
		...
		private void setPlayersBack()
		{
			...
			BlockCheckerRestriction.getInstance().deactivate();
		}
