...
import net.l2emuproject.gameserver.events.global.fortsiege.FortSiegeManager.SiegeSpawn;
...
public class FortSiege extends AbstractSiege
{
	...
	public List<L2Player> getOwnersInZone()
	{
		...
		for (L2Character cha : getZone().getCharactersInside())
		{
			if (cha instanceof L2Player && ((L2Player) cha).getClan() != null && ((L2Player) cha).getClan() == getFort().getOwnerClan())
				lst.add((L2Player) cha);
		}
		return lst;
	}
