 ...
 */
package net.l2emuproject.gameserver.geodata;
...
import net.l2emuproject.gameserver.geodata.pathfinding.cellnodes.CellPathFinding;
...
final class GeoEngine extends GeoData
{
	...
	private final Map<Integer, Map<Long, Byte>> _instanceGeodata 	= new FastMap<Integer, Map<Long, Byte>>().shared();
	...
	private void nInitGeodata()
	{
		...
		try
		{
			...
			// L2EMU_ADD
			File Data = new File(Config.GEODATA_ROOT, "/geodata/geo_index.txt");
			// L2EMU_ADD
			if (!Data.exists())
				return;
	...
	public boolean loadGeodataFile(byte rx, byte ry)
	{
		...
		// L2EMU_ADD
		File Geo = new File(Config.GEODATA_ROOT, fname);
		// L2EMU_ADD
		...
		try
		{
			...
			if (_log.isDebugEnabled())
				_log.debug("Geo Engine: - Max Layers: " + flor + " Size: " + size + " Loaded: " + index);
		}
		catch (Exception e)
		{
