...
import net.l2emuproject.config.ConfigProperty;
import net.l2emuproject.config.L2Properties;
import net.l2emuproject.io.RedirectingOutputStream.BufferedRedirectingOutputStream;
...
public abstract class L2Config
{
	...
	static
	{
		...
		{
			System.out.print("L2EmuProject servers should not run under root-account ... exited.");
			System.exit(-1);
		}
		...
		System.setProperty("java.util.logging.manager", "net.l2emuproject.util.logging.L2LogManager");
	...
	public static Set<String> getAllowedTelnetHostAddresses()
	{
		...
		try
		{
			for (String host : new L2Properties(L2Config.NETWORK_FILE).getProperty("ListOfHosts").split(","))
			{
				try
				{
