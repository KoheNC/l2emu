 ...
 */
public class HallOfSuffering extends QuestJython
{
	...
	private static final int[][]	TWIN_SPAWNS				=
															{
															{ 25665, -173727, 218169, -9536 },
															{ 25666, -173727, 218049, -9536 } };
	...
	private void runTumors(HSWorld world)
	{
		...
		L2Npc mob = addSpawn(TUMOR_ALIVE, TUMOR_SPAWNS[world.status][0], TUMOR_SPAWNS[world.status][1], TUMOR_SPAWNS[world.status][2], 0, false, 0, false,
				world.instanceId);
		mob.disableCoreAI(true);
		mob.setIsImmobilized(true);
		mob.getStatus().setCurrentHp(mob.getMaxHp() * 0.5);
	...
	public String onAdvEvent(String event, L2Npc npc, L2Player player)
	{
		...
		if (tmpworld instanceof HSWorld)
		{
			...
			if (event.equalsIgnoreCase("spawnBossGuards"))
			{
				...
				L2Npc mob = addSpawn(TWIN_MOBIDS[Rnd.get(TWIN_MOBIDS.length)], TWIN_SPAWNS[0][1], TWIN_SPAWNS[0][2], TWIN_SPAWNS[0][3], 0, false, 0, false,
						npc.getInstanceId());
				((L2Attackable) mob).addDamageHate(((L2Attackable) npc).getMostHated(), 0, 1);
				if (Rnd.get(100) < 33)
				{
					mob = addSpawn(TWIN_MOBIDS[Rnd.get(TWIN_MOBIDS.length)], TWIN_SPAWNS[1][1], TWIN_SPAWNS[1][2], TWIN_SPAWNS[1][3], 0, false, 0, false,
							npc.getInstanceId());
					((L2Attackable) mob).addDamageHate(((L2Attackable) npc).getMostHated(), 0, 1);
				}
			...
			else if (event.equalsIgnoreCase("ressurectTwin"))
			{
				...
				npc.doRevive();
				npc.doCast(skill);
				npc.getStatus().setCurrentHp(aliveTwin.getCurrentHp());
