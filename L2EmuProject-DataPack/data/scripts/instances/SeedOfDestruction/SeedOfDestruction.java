...
public final class SeedOfDestruction extends QuestJython
{
	...
	private static final int			TIAT_BODY_GUARD				= 29162;
	...
	{
		if (npc.getNpcId() == TIAT_BODY_GUARD)
			startQuestTimer("BodyGuardThink", 2500 + Rnd.get(-200, 200), npc, null, true);
		else
			npc.disableCoreAI(true);
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (tmpworld instanceof SODWorld)
		{
			...
			{
				L2PcInstance target = L2World.getInstance().getPlayer(Rnd.get(world.allowed.size()));
				if (world.deviceSpawnedMobCount < MAX_DEVICESPAWNEDMOBCOUNT && target != null && target.getInstanceId() == npc.getInstanceId()
						&& !target.isDead())
				{
					L2Attackable mob = (L2Attackable) addSpawn(SPAWN_MOB_IDS[Rnd.get(SPAWN_MOB_IDS.length)], npc.getSpawn().getLocx(),
							npc.getSpawn().getLocy(), npc.getSpawn().getLocz(), npc.getSpawn().getHeading(), false, 0, false, world.instanceId);
			...
			else if (event.equalsIgnoreCase("BodyGuardThink"))
			{
				...
				if (mostHate != null)
				{
					double dist = Util.calculateDistance(mostHate.getXdestination(), mostHate.getYdestination(), mostHate.getZdestination(), npc.getSpawn()
							.getLocx(), npc.getSpawn().getLocy(), npc.getSpawn().getLocz(), true);
					if (dist > 1200)
	...
	public final String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		if (tmpworld instanceof SODWorld)
		{
			...
			else if (world.status >= 7)
			{
				...
				else if (npc.getNpcId() == TIAT_BODY_GUARD)
				{
					cancelQuestTimer("BodyGuardThink", npc, null);
					L2Attackable mob = (L2Attackable) addSpawn(npc.getNpcId(), npc.getSpawn().getLocx(), npc.getSpawn().getLocy(), npc.getSpawn().getLocz(),
							npc.getSpawn().getHeading(), false, 0, false, world.instanceId);
