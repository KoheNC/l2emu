...
public final class WintermelonWoes extends Quest
{
	...
	@Override
	public String onAttack(final L2Npc npc, final L2Player attacker, final int damage, final boolean isPet)
	{
		if (ArrayUtils.contains(WATERMELON_LIST, npc.getNpcId()))
		{
			if (isPet)
			{
				noChronoText(npc);
	...
	@Override
	public String onSkillSee(final L2Npc npc, final L2Player caster, final L2Skill skill, final L2Object[] targets, final boolean isPet)
	{
		if (ArrayUtils.contains(targets, npc) && ArrayUtils.contains(WATERMELON_LIST, npc.getNpcId()) && (skill.getId() == NECTAR_SKILL))
		{
			switch (npc.getNpcId())
			{
				case 13271:
	...
	@Override
	public String onKill(final L2Npc npc, final L2Player killer, final boolean isPet)
	{
		dropItem(npc, killer);
	...
	private void dropItem(final L2Npc mob, final L2Player player)
	{
	...
	private void randomSpawn(final int npcId, final L2Npc npc, final boolean delete)
	{
		if (Rnd.get(100) < 10)
			spawnNext(npcId, npc);
		else
	...
	private void ChronoText(final L2Npc npc)
	{
		if (Rnd.get(100) < 20)
	...
	private void noChronoText(final L2Npc npc)
	{
		if (Rnd.get(100) < 20)
			npc.broadcastPacket(new CreatureSay(npc.getObjectId(), SystemChatChannelId.Chat_Normal, npc.getName(), _NOCHRONO_TEXT[Rnd
					.get(_NOCHRONO_TEXT.length)]));
	}

	private void nectarText(final L2Npc npc)
	{
		if (Rnd.get(100) < 30)
	...
	private void spawnNext(final int npcId, final L2Npc npc)
	{
	...
	public String onFirstTalk(final L2Npc npc, final L2Player player)
	{
		...
		if (st == null)
		{
			final Quest q = QuestService.getInstance().getQuest(getName());
