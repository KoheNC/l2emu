...
public class L2NpcAIScript extends Quest {
	...
	public final void removeCoords(final int npcId) {
		if(_coords.containsKey(npcId)) { _coords.remove(npcId); }
	}
		...
		if(_c != null && ni._canMove) {
			if(ni._movePointIndex < _c.length) {
					ThreadPoolManager.getInstance().execute(new Runnable() {
						@Override
						public void run() {
							npc.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(new Location(_c[ni._movePointIndex][0], _c[ni._movePointIndex][1], _c[ni._movePointIndex][2])));
							++ni._movePointIndex;
						}
					});
	...
	private boolean notifyNpcRangeEnter(final L2Character character, final L2Npc npc) {
		...
		if(player != null) { return showResult(player, res); }
		return true;
	}
	...
	private class NpcRangeTask implements Runnable {
		...
		public NpcRangeTask(final L2Npc npc, final int radius) { _npc = npc; _radius = radius; }
		@Override
		public void run() {
	...
	{
		if(res == null || player == null) { return true; }
		if(res.endsWith(".htm")) { showHtmlFile(player, res); }
