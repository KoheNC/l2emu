...
public class FeedableBeasts extends L2AttackableAIScript
{
	...
	public void spawnNext(L2Npc npc, int growthLevel, L2PcInstance player, int food)
	{
		...
		if (contains(TAMED_BEASTS,nextNpcId))
		{
			...
			{
				npc.broadcastPacket(new NpcSay(nextNpc, player.getName()+", whenever I look at spice, I think about you."));
			}
			else if (rand == 2)
			{
				npc.broadcastPacket(new NpcSay(nextNpc, player.getName()+", you do not need to return to the village. I will give you strength"));
			}
			else if (rand == 3)
			{
