...
public final class OracleTeleport extends QuestJython
{
	private static final String		QN				= "OracleTeleport";
	...
	public final String onAdvEvent(String event, L2Npc npc, L2Player player)
	{
		...
		{
			if (ArrayUtils.contains(TEMPLE_PRIEST, npcId) && st.getState() == State.STARTED)
			{
			...
			else if (ArrayUtils.contains(RIFT_POSTERS, npcId) && st.getState() == State.STARTED)
			{
				int x = RETURN_LOCS[st.getInt(ID)][0];
				int y = RETURN_LOCS[st.getInt(ID)][1];
				int z = RETURN_LOCS[st.getInt(ID)][2];
		...
		else if (event.equalsIgnoreCase("Festival"))
		{
			...
			if (ArrayUtils.contains(TOWN_DAWN, id))
			{
				player.teleToLocation(-80157, 111344, -4901);
				player.setIsIn7sDungeon(true);
			}
			else if (ArrayUtils.contains(TOWN_DUSK, id))
			{
	...
	public final String onTalk(L2Npc npc, L2Player player)
	{
		...
		if (ArrayUtils.contains(TOWN_DAWN, npcId))
		{
			st.setState(State.STARTED);
			int i = 0;
		...
		if (ArrayUtils.contains(TOWN_DUSK, npcId))
		{
			st.setState(State.STARTED);
			int i = 0;
	...
	{
		new OracleTeleport(-1, QN, "teleports");
	}
}
