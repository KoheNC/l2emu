...
public class AirShipGludioGracia extends Quest implements Runnable
{
	...
	private final void broadcastInGludio(String msg)
	{
		...
		if (_atcGludio != null)
			_atcGludio.broadcastPacket(new NpcSay(_atcGludio.getObjectId(), SystemChatChannelId.Chat_Shout.getId(), _atcGludio.getNpcId(), msg));
	}
	...
	private final void broadcastInGracia(String msg)
	{
		...
		if (_atcGracia != null)
			_atcGracia.broadcastPacket(new NpcSay(_atcGracia.getObjectId(), SystemChatChannelId.Chat_Shout.getId(), _atcGracia.getNpcId(), msg));
	}
