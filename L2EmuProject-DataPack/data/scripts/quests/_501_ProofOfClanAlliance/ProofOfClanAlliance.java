...
public final class ProofOfClanAlliance extends QuestJython
{
	...
	private ProofOfClanAlliance(int questId, String name, String descr)
	{
		...
		questItemIds = new int[]
		{
				...
				/*HERB_OF_HARIT, HERB_OF_VANOR, HERB_OF_OEL_MAHUM,*/BLOOD_OF_EVA,
				SYMBOL_OF_LOYALTY,
	...
	public final String onAdvEvent(String event, L2Npc npc, L2Player player)
	{
		...
		if (event.startsWith("loyalty"))
		{
			...
			if (qc.isLoyal(player))
				player.addItem(QN, SYMBOL_OF_LOYALTY, 1, npc, true);
			else if (DEBUG)
		...
		else
		{
			...
			else
			{
				...
				else if ("30757-05.htm".equals(event))
				{
					...
					else
					{
						...
						else
						{
							...
							npc.broadcastPacket(new MagicSkillUse(npc, player, skill, skill.getHitTime(), skill.getReuseDelay()));
				...
				else if ("30758-07.htm".equals(event))
				{
					if (_minigame == null)
					{
						if (player.reduceAdena(QN, 10000, npc, true))
						{
							qc.getChests().reset();
							return "30758-08.htm";
						}
	...
	public final String onKill(L2Npc npc, L2Player killer, boolean isPet)
	{
		...
		if (qc == null || !qc.checkLeader() || qc.getPart() != KALIS || !qc.isLoyal(killer))
			return null;
		QuestState qs = killer.getQuestState(QN);
		switch (npc.getNpcId())
		{
	...
	public final String onTalk(L2Npc npc, L2Player talker)
	{
		...
		switch (npc.getNpcId())
		{
			...
			case KALIS:
				...
				{
					QuestState l = qc.getClan().getLeader().getPlayerInstance().getQuestState(QN);
				...
				else if (state == State.STARTED)
				{
					...
					else if (part == KALIS * 2 && qs.getQuestItemsCount(HERB_OF_HARIT) > 0 && qs.getQuestItemsCount(HERB_OF_OEL_MAHUM) > 0
	...
	private final String buildReply(L2Npc npc, int answer)
	{
	...
	{
		new ProofOfClanAlliance(501, QN, "Proof of Clan Alliance");
	}

	private final class QuestClan
	{
		...
		public synchronized boolean addLoyalMember(L2Player player)
		{
			...
			{
				if (DEBUG)
					_log.warn("A new loyal member " + player + " does not belong to " + this, new IllegalArgumentException());
				return false;
			}
			for (int i = 0; i < NEEDED_MEMBERS; i++)
			{
