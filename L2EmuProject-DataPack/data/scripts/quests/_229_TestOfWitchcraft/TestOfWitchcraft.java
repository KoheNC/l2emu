 ...
 * Remade by lewzer
 */
public final class TestOfWitchcraft extends QuestJython
{
	...
	public TestOfWitchcraft(int questId, String name, String descr)
	{
		...
		switch (npcId)
		{
			case ORIM:
				if (st.getQuestItemsCount(MARK_OF_WITCH_CRAFT) != 0)
				{
					...
					{
						case 0:
							if (st.getPlayer().getClassId().getId() == 0x0b || st.getPlayer().getClassId().getId() == 0x04
									|| st.getPlayer().getClassId().getId() == 0x20)
							{
			...
			case IKER:
				if (cond == 2)
				{
					...
					else if (st.getQuestItemsCount(IKERS_LIST) > 0
							&& (st.getQuestItemsCount(DIRE_WYRM_FANG) < 20 || st.getQuestItemsCount(LETO_LIZARDMAN_CHARM) < 20 || st
									.getQuestItemsCount(ENCHANTED_GOLEM_HEARTSTONE) < 20))
					{
			...
			case NESTLE:
				if (cond == 2)
				{
					if (st.getQuestItemsCount(AKLANTOOTH_1STGEM) > 0 && st.getQuestItemsCount(AKLANTOOTH_2NDGEM) > 0
							&& st.getQuestItemsCount(AKLANTOOTH_3RDGEM) > 0)
					{
	...
	public final String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		else if (npcId == ENCHANTED_STONE_GOLEM && 80 >= Rnd.get(100) && cond == 2 && st.getQuestItemsCount(ENCHANTED_GOLEM_HEARTSTONE) < 20
				&& st.getQuestItemsCount(IKERS_LIST) > 0)
		{
			st.giveItems(ENCHANTED_GOLEM_HEARTSTONE, 1);
			st.sendPacket(SND_ITEM_GET);
		}
		else if (npcId == LETO_LIZARDMAN && 50 >= Rnd.get(100) && cond == 2 && st.getQuestItemsCount(LETO_LIZARDMAN_CHARM) < 20
				&& st.getQuestItemsCount(IKERS_LIST) > 0)
		{
			st.giveItems(LETO_LIZARDMAN_CHARM, 1);
			st.sendPacket(SND_ITEM_GET);
		}
		else if (npcId == LETO_LIZARDMAN_ARCHER && 50 >= Rnd.get(100) && cond == 2 && st.getQuestItemsCount(LETO_LIZARDMAN_CHARM) < 20
				&& st.getQuestItemsCount(IKERS_LIST) > 0)
		{
			st.giveItems(LETO_LIZARDMAN_CHARM, 1);
			st.sendPacket(SND_ITEM_GET);
		}
		else if (npcId == LETO_LIZARDMAN_SOLDIER && 60 >= Rnd.get(100) && cond == 2 && st.getQuestItemsCount(LETO_LIZARDMAN_CHARM) < 20
				&& st.getQuestItemsCount(IKERS_LIST) > 0)
		{
			st.giveItems(LETO_LIZARDMAN_CHARM, 1);
			st.sendPacket(SND_ITEM_GET);
		}
		else if (npcId == LETO_LIZARDMAN_WARRIOR && 65 >= Rnd.get(100) && cond == 2 && st.getQuestItemsCount(LETO_LIZARDMAN_CHARM) < 20
				&& st.getQuestItemsCount(IKERS_LIST) > 0)
		{
			st.giveItems(LETO_LIZARDMAN_CHARM, 1);
			st.sendPacket(SND_ITEM_GET);
		}
		else if (npcId == LETO_LIZARDMAN_SHAMAN && 70 >= Rnd.get(100) && cond == 2 && st.getQuestItemsCount(LETO_LIZARDMAN_CHARM) < 20
				&& st.getQuestItemsCount(IKERS_LIST) > 0)
		{
			st.giveItems(LETO_LIZARDMAN_CHARM, 1);
			st.sendPacket(SND_ITEM_GET);
		}
		else if (npcId == LETO_LIZARDMAN_OVERLORD && 70 >= Rnd.get(100) && cond == 2 && st.getQuestItemsCount(LETO_LIZARDMAN_CHARM) < 20
				&& st.getQuestItemsCount(IKERS_LIST) > 0)
		{
			st.giveItems(LETO_LIZARDMAN_CHARM, 1);
			st.sendPacket(SND_ITEM_GET);
		...
		else if (npcId == TAMLIN_ORC && 50 >= Rnd.get(100) && cond == 6 && st.getQuestItemsCount(TAMLIN_ORC_AMULET) < 20
				&& st.getQuestItemsCount(VADINS_CRUCIFIX) > 0)
		{
			st.giveItems(TAMLIN_ORC_AMULET, 1);
			st.sendPacket(SND_ITEM_GET);
		}
		else if (npcId == TAMLIN_ORC_ARCHER && 55 >= Rnd.get(100) && cond == 6 && st.getQuestItemsCount(TAMLIN_ORC_AMULET) < 20
				&& st.getQuestItemsCount(VADINS_CRUCIFIX) > 0)
		{
			st.giveItems(TAMLIN_ORC_AMULET, 1);
			st.sendPacket(SND_ITEM_GET);
		...
		{
			if (st.getQuestItemsCount(AKLANTOOTH_4THGEM) == 0 && 50 >= Rnd.get(100))
			{
				st.giveItems(AKLANTOOTH_4THGEM, 1);
			}
			if (st.getQuestItemsCount(AKLANTOOTH_5THGEM) == 0 && 50 >= Rnd.get(100))
			{
				st.giveItems(AKLANTOOTH_5THGEM, 1);
			}
			if (st.getQuestItemsCount(AKLANTOOTH_6THGEM) == 0 && 50 >= Rnd.get(100))
			{
		...
		{
			if (player.getActiveWeaponItem().getItemId() == SWORD_OF_BINDING)
			{
	...
	{
		new TestOfWitchcraft(229, QN, "Test of Witchcraft");
	}
}