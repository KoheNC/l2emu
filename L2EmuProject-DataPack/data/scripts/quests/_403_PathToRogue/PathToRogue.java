...
public final class PathToRogue extends QuestJython
{
		...
		if (npcId == BEZIQUE)
		{
			...
			else if (qs.getQuestItemsCount(HORSESHOE_OF_LIGHT) == 0)
			{
				if (allStolenItems(qs))
				{
					...
					return "30379-07.htm";
				else if (qs.getQuestItemsCount(NETIS_BOW) != 0 && qs.getQuestItemsCount(NETIS_DAGGER) != 0 && qs.getQuestItemsCount(WANTED_BILL) == 0)
					return "30379-10.htm";
				else
					return "30379-11.htm";
			}
	...
	private final boolean allStolenItems(QuestState qs)
	{
		return (qs.getQuestItemsCount(STOLEN_JEWELRY) + qs.getQuestItemsCount(STOLEN_TOMES) + qs.getQuestItemsCount(STOLEN_RING)
				+ qs.getQuestItemsCount(STOLEN_NECKLACE) == 4);
	}

	public static void main(String[] args)
	{
		new PathToRogue(403, QN, "Path to Rogue");
	}
}
