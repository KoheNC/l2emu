...
import net.l2emuproject.loginserver.beans.BanInfo;
import net.l2emuproject.tools.network.SubNet;
...
public class BanManager
{
	...
	/** Banned ips */
	private final FastMap<SubNet, BanInfo>	_bannedIps		= new FastMap<SubNet, BanInfo>().shared();
	private final FastMap<SubNet, BanInfo>	_restrictedIps	= new FastMap<SubNet, BanInfo>().shared();
	...
	public void load()
	{
		try
		{
			...
			final File file = new File(BAN_LIST);
	...
	public void addBanForAddress(String address, long expiration) throws UnknownHostException
	{
		...
		final SubNet _net = new SubNet(netAddress.getHostAddress());
		if (expiration != 0)
	...
	{
		final SubNet _net = new SubNet(address);
		if (duration != 0)
	...
	{
		final SubNet _net = new SubNet(address.getHostAddress());
		if (duration != 0)
			_bannedIps.put(_net, new BanInfo(_net, System.currentTimeMillis() + duration));
		else
			_restrictedIps.put(_net, new BanInfo(_net, 0));
	...
	{
		for (final Map.Entry<SubNet, BanInfo> _bannedIP : _restrictedIps.entrySet())
		{
			final SubNet net = _bannedIP.getKey();

			if (net != null && net.isInSubnet(address.getHostAddress()))
			{
				final BanInfo bi = _bannedIP.getValue();
				if (bi != null)
					return true;
			}
		}
	...
	{
		final Entry<SubNet, BanInfo> bannedIP = getInfo(address);
		if (bannedIP != null)
	...
	{
		for (final Map.Entry<SubNet, BanInfo> _bannedIP : _bannedIps.entrySet())
		{
			final SubNet net = _bannedIP.getKey();
